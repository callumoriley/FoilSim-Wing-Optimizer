...
// Modified property ranges
const angleLow = -20.0;
const angleHigh = 20.0;
const camberLow = -20.0;
const camberHigh = 20.0;
const thicknessLow = 1.0;
const thicknessHigh = 20.0;

// All the lift and drag code is from this site: https://www.grc.nasa.gov/www/k-12/airplane/foil3.html

function optimizationFunction() {
    return eval(document.getElementById("optiFunc").value);
}
function traverseOptimization() {
    // Optimize each parameter individually
    // Ultimately optimizing lift/drag ratio

    // Optimize angle
    var angleVariations = [];
    var angles = [];
    for (angle = angleLow; angle <= angleHigh; angle += STEP_VAL) {
        if (calculateDrag() > ZERO_THRESHOLD && calculateLift() > ZERO_THRESHOLD) {
            angleVariations.push(optimizationFunction());
            angles.push(angle);
        }
    }
    angle = angles[angleVariations.indexOf(Math.max(...angleVariations))];

    // Optimize camber
    var camberVariations = [];
    var cambers = [];
    for (camber = camberLow; camber <= camberHigh; camber += STEP_VAL) {
        if (calculateDrag() > ZERO_THRESHOLD && calculateLift() > ZERO_THRESHOLD) {
            camberVariations.push(optimizationFunction());
            cambers.push(camber);
        }
    }
    camber = cambers[camberVariations.indexOf(Math.max(...camberVariations))];

    // Optimize thickness
    var thicknessVariations = [];
    var thicknesses = [];
    for (thickness = thicknessLow; thickness <= thicknessHigh; thickness += STEP_VAL) {
        if (calculateDrag() > ZERO_THRESHOLD && calculateLift() > ZERO_THRESHOLD) {
            thicknessVariations.push(optimizationFunction());
            thicknesses.push(thickness);
        }
    }
    thickness = thicknesses[thicknessVariations.indexOf(Math.max(...thicknessVariations))];

    var angleLow2 = angle + angleLow / SECOND_PASS_FACTOR;
    var angleHigh2 = angle + angleHigh / SECOND_PASS_FACTOR;
    var camberLow2 = camber + camberLow / SECOND_PASS_FACTOR;
    var camberHigh2 = camber + camberHigh / SECOND_PASS_FACTOR;
    var thicknessLow2 = thickness - thicknessHigh / SECOND_PASS_FACTOR;
    var thicknessHigh2 = thickness + thicknessHigh / SECOND_PASS_FACTOR;

    var variations = [];
    var variationParameters = [];
    
    for (angle = angleLow2; angle <= angleHigh2; angle += STEP_VAL) {
        for (camber = camberLow2; camber <= camberHigh2; camber += STEP_VAL) {
            for (thickness = thicknessLow2; thickness <= thicknessHigh2; thickness += STEP_VAL) {
                if (calculateDrag() > ZERO_THRESHOLD && calculateLift() > ZERO_THRESHOLD) {
                    variations.push(optimizationFunction());
                    variationParameters.push([angle, camber, thickness]);
                }
            }
        }
    }

    var outputParams = variationParameters[variations.indexOf(Math.max(...variations))];
    angle = outputParams[0];
    camber = outputParams[1];
    thickness = outputParams[2];

    writeValues();
}

function writeValues() {
    document.getElementById("angleOut").innerHTML = angle.toString();
    document.getElementById("camberOut").innerHTML = camber.toString();
    document.getElementById("thicknessOut").innerHTML = thickness.toString();
    document.getElementById("liftOut").innerHTML = calculateLift().toString();
    document.getElementById("dragOut").innerHTML = calculateDrag().toString();
    document.getElementById("ldrOut").innerHTML = calculateLDRatio().toString();
}
...
function getrVal(){
    var ycval = getycVal();
    var rval = thickness/100.0 + Math.sqrt((thickness*thickness)/(16.0) + (ycval*ycval) + 1.0);
...
function calculateLift(){ // -------------------------------------------------- FUNCTION TO CALCULATE LIFT ----------------------------------------------------------
    //obtain the inputs 
    const pi = 3.1415926;
...
function calculateDrag(){ // ------------------------------------------------------ FUNCTION TO CALCULATE DRAG ------------------------------------------------------
